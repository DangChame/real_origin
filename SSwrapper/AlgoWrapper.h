...
/* @param in : 12 byte long auth_initials array
   @param out: 12 byte long out_auth_initials array*/
MXM_ALGOSUITE_API void mxm_algosuite_manager_getauthinitials( const uint8_t *const auth_inits,
										    				  uint8_t *out_auth_initials);

/* @param in : 12 byte long auth_str1 array - 12 byte long remote public of sensorhub
   @param in : 32 byte long auth_str2 array - 32 byte long sensorhub final authentication string*/
MXM_ALGOSUITE_API void mxm_algosuite_manager_authenticate( const uint8_t *const auth_str1,
